...
const pains = ["見積もり作成に時間がかかる", "Excel・紙・FAXを減らしたい", "人によってやり方が違う", "既存システムが使いづらい"];
...
export default function Home({ initialTarget }: { initialTarget?: string }) {
  ...
  function submit(e: FormEvent<HTMLFormElement>) { e.preventDefault(); if (e.currentTarget.reportValidity()) setSent(true); }

  return <main>
    <header className="nav wrap"><a className="brand" href="#top" aria-label="会話型AI見積もりツール ホーム">会話型AI見積もり <small>CONVERSATIONAL AI ESTIMATE</small></a><nav aria-label="ページ内メニュー"><a href="#benefits">相談でわかること</a><a href="#challenge">課題</a><a href="#solution">解決方法</a><a href="#flow">進め方</a><a href="#comparison">他との違い</a><a href="#faq">よくある質問</a><a className="navCta" href="#contact">無料で相談する</a></nav></header>

    <section className="hero" id="top"><div className="heroGlow" /><div className="wrap heroGrid"><div className="heroCopy"><p className="eyebrow">{segment.area} × {segment.industry}の業務改善</p><h1>{SEGMENTS.default.hero}</h1><p className="lead">業務の話を約30分聞かせてください。AIと人で、任せられる仕事・最初につくる仕組み・概算費用を無料で整理します。</p><div className="trust"><span>✓ 相談無料</span><span>✓ 3分で入力</span><span>✓ 営業電話なし</span></div><p className="micro">何を作るか決まっていなくても大丈夫です。</p></div><div className="formCard" id="contact">{!sent ? <form onSubmit={submit}><div className="formHead"><span className="statusDot" />無料業務整理・概算相談</div><h2>まずは、お困りの業務を教えてください</h2><p className="formNote">内容をもとに、担当者からメールで日程をご案内します。</p><fieldset><legend>どんなことで困っていますか？</legend>{pains.map((v, i) => <label className="choice" key={v}><input type="radio" name="pain" value={v} required={i === 0} onChange={() => setOther(false)} /><span>{v}</span></label>)}<label className="choice"><input type="radio" name="pain" value="other" onChange={() => setOther(true)} /><span>その他・まだ整理できていない</span></label></fieldset>{other && <textarea name="other" aria-label="困りごとの詳細" placeholder="例：毎月の集計に時間がかかる、担当者しか見積もれない…" rows={2} />}<label className="mailLabel">会社のメールアドレス<input name="email" type="email" required placeholder="name@company.jp" /></label><input type="hidden" name="segment" value={segmentKey} /><button type="submit">無料で業務と費用感を整理する <b>→</b></button><small>送信で契約になることはありません。無理な営業も行いません。</small></form> : <div className="thanks" role="status"><div className="check">✓</div><h2>ご相談を受け付けました</h2><p>内容を確認し、メールで日程をご案内します。</p><button onClick={() => setSent(false)}>入力内容を変更する</button></div>}</div></div></section>

    <section className="benefits" id="benefits"><div className="wrap"><p className="eyebrow dark">無料相談でわかること</p><h2>相談のあと、<em>次に進むための材料</em>をお渡しします。</h2><div className="benefitGrid"><article><span>01</span><h3>どこで仕事が止まるか</h3><p>時間がかかる作業や、特定の人に頼っている判断を整理します。</p></article><article><span>02</span><h3>最初に仕組みにする仕事</h3><p>すべてを変えるのではなく、効果が出やすい一歩目を一緒に考えます。</p></article><article><span>03</span><h3>費用の目安</h3><p>要件に合わせ、無理なく始める場合の概算費用をレンジでお伝えします。</p></article></div></div></section>

    <section className="proof"><div className="wrap proofInner"><div className="proofLabel"><p>まずは、困っている仕事を整理します</p><h2>{segment.area}の{segment.industry}では、<em>{segment.example}</em>のご相談が多いです。</h2></div><div className="proofTags">{segment.proof.map((item, index) => <div key={item}><b>0{index + 1}</b><span>{item}</span></div>)}</div></div></section>
    ...
    <section className="solutionStory wrap section" id="solution"><p className="eyebrow dark">私たちがすること</p><h2>まずは、<em>困っている仕事</em>を聞かせてください。</h2><p className="sectionLead">「何を作るか」が決まっていなくても大丈夫です。AIが質問し、会話を要件へ整理します。解決の方向・費用の幅・完成後の画面まで見える状態にします。</p><div className="uiShowcase"><figure><img src="/ai-image1.png" alt="AIが業務の困りごとを聞く会話画面" /><figcaption><b>01 / AIヒアリング</b><span>答えるほど、課題の輪郭が見えてくる</span></figcaption></figure><figure><img src="/ai-image2.png" alt="ヒアリング結果として概算見積もりと画面案が表示される結果画面" /><figcaption><b>02 / 結果画面</b><span>費用レンジと完成後の画面を同時に確認</span></figcaption></figure></div><div className="caseLabel"><p>ご相談の例</p><h3>{segment.area}の{segment.industry}で想定する、最初の仕組み化</h3></div><div className="beforeAfter"><div className="quote">「{useCase.from}」</div><div className="arrow">→</div><div className="solution"><span>最初のご提案例</span><strong>{useCase.to}</strong><p>必要な入力・画面案・費用レンジまで整理します。</p></div></div></section>

    <section className="flow section" id="flow"><div className="wrap"><p className="eyebrow">ご相談から提案までの流れ</p><h2>答えるだけで、解決のかたちが見えてくる。</h2><div className="steps"><article><span>01</span><div className="stepIcon">？</div><h3>困りごとを教える</h3><p>業種や人数、いまの作業を選択式で回答。まとまった仕様書は不要です。</p></article><article><span>02</span><div className="stepIcon">✦</div><h3>AIと人が整理する</h3><p>業務の流れを確認し、AIでできること・人が担うことを整理します。</p></article><article><span>03</span><div className="stepIcon">¥</div><h3>概算と画面案を受け取る</h3><p>初期費用の目安と、完成後の画面イメージをご提案します。</p></article></div></div></section>

    <section className="comparison section" id="comparison"><div className="wrap"><p className="eyebrow dark">他の選択肢との違い</p><h2>既存の選択肢では、<em>相談の入口</em>が埋まらない。</h2><p className="sectionLead">何を作るか説明できる人向けのサービスはあります。けれど、困りごとを話すところから始め、要件・費用・画面案まで受け取れる選択肢は多くありません。</p><div className="landscapeFlow"><article><span>01</span><p>見積もりSaaS</p><h3>仕様を入力して、工数を出す。</h3><small>最初から必要な機能を整理できる会社向け。</small></article><article><span>02</span><p>AI開発サービス</p><h3>会話から、開発へ進む。</h3><small>業務に合わせた要件整理や伴走は、別途必要になりやすい。</small></article><article><span>03</span><p>簡易見積もりLP</p><h3>フォーム入力で、概算を出す。</h3><small>入力項目が決まっており、壁打ちや画面案には届きにくい。</small></article></div><div className="gapStatement"><div><span>私たちが埋める空白</span><strong>困りごとを話すだけで、要件・費用・画面案まで見えてくる。</strong></div><ol><li><b>01</b> AIが質問する</li><li><b>02</b> 要件に整理する</li><li><b>03</b> 人が相談に乗る</li></ol></div><p className="comparisonNote">※ 一般的な提供形態をもとにした整理です。各サービスの内容は変更される場合があります。</p></div></section>
    ...
    <section className="consultation section" id="consultation"><div className="wrap"><p className="eyebrow dark">ご相談の進め方</p><h2>まずは、<em>業務の話</em>を聞かせてください。</h2><div className="consultationGrid"><article><span>01</span><h3>約30分、お話を伺います</h3><p>困っている作業や、今のやり方をお聞きします。仕様書の準備は不要です。</p></article><article><span>02</span><h3>できることを整理します</h3><p>仕組みにする仕事、AIで補えること、概算費用の考え方を一緒に整理します。</p></article><article><span>03</span><h3>進め方は内容を見て決められます</h3><p>開発の依頼は必須ではありません。詳しい業務の共有前に、NDAを含めた進め方もご相談いただけます。</p></article></div><p className="operatorNote">運営：KACHI CREATION INC. ／ オンラインで全国対応。現場確認が必要な場合は、地域に応じてご相談ください。</p></div></section>

    <section className="faq section" id="faq"><div className="wrap"><p className="eyebrow dark">よくあるご質問</p><h2>ご相談の前に</h2>{[["本当に無料ですか？", "初回のヒアリング、課題整理、概算見積もりまでは無料です。開発を依頼するかは、内容を見てから決められます。"],["まだ何を作りたいか決まっていません。", "問題ありません。「どの作業が大変か」から伺い、ツール化すべきかも含めて一緒に考えます。"],["地方でも対応できますか？", "オンラインで全国対応しています。現場確認が必要な場合は、地域に応じて個別にご相談します。"],["相談後に営業されませんか？", "ご希望のない継続的な営業連絡は行いません。見積もりだけのご相談も歓迎です。"]].map(([q, a]) => <details key={q}><summary>{q}<span>＋</span></summary><p>{a}</p></details>)}</div></section>

    <section className="bottomCta"><div className="wrap"><p>課題がまとまっていなくても、大丈夫です。</p><h2>{segment.pain}を、一度整理してみませんか。</h2><a href="#contact">無料見積もりを依頼する <b>→</b></a><small>入力は約3分・相談だけでもOK</small></div></section>
    {!floatDismissed && <aside className={`yFloat ${floatVisible ? "isVisible" : ""} ${floatPulse ? "isPulse" : ""}`} aria-label="無料見積もりと無料相談"><a className="floatLink floatEstimate" href="#contact"><small>3分・入力無料</small><strong>無料見積もり</strong></a><a className="floatLink floatContact" href="#contact"><small>オンライン対応</small><strong>無料相談 →</strong></a><button className="floatClose" type="button" aria-label="バナーを閉じる" onClick={() => { setFloatVisible(false); setFloatDismissed(true); sessionStorage.setItem("ai-estimate:float-dismissed", "1"); }}>✕</button></aside>}
    <footer><div className="wrap footerInner"><div><a className="brand" href="#top">会話型AI見積もり <small>CONVERSATIONAL AI ESTIMATE</small></a><p>地域企業のための、伴走型AI業務改善</p></div><nav className="footerLinks" aria-label="フッターメニュー"><a href="#benefits">相談でわかること</a><a href="#challenge">課題</a><a href="#solution">解決方法</a><a href="#flow">進め方</a><a href="#comparison">他との違い</a><a href="#consultation">ご相談の進め方</a><a href="#faq">よくある質問</a><a href="#contact">無料相談</a></nav><small>© 2026 KACHI CREATION INC.</small></div></footer>
  </main>;
}
