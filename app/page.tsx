...
export default function Home({ initialTarget }: { initialTarget?: string }) {
  ...
  return <main>
    ...
    <section className="hero" id="top"><div className="heroGlow" /><div className="wrap heroGrid"><div className="heroCopy"><p className="eyebrow">{segment.area} × {segment.industry}の業務改善</p><h1>{SEGMENTS.default.hero}</h1><p className="lead">まずは、お困りの業務を教えてください。AIと人が、仕組みにできる仕事・最初に整える業務・費用の目安を無料で整理します。</p><div className="trust"><span>✓ 相談無料</span><span>✓ 入力は約3分</span><span>✓ しつこい営業なし</span></div><p className="micro">何を作るか決まっていなくても、相談できます。</p></div><div className="formCard googleFormCard" id="contact"><div className="formHead"><span className="statusDot" />無料相談・概算費用のご案内</div><h2>お困りの業務を教えてください</h2><p className="formNote">Googleフォームで、いま困っている業務を教えてください。内容を確認後、メールでご相談日時をご案内します。</p><a className="googleFormButton" href={GOOGLE_FORM_URL} target="_blank" rel="noreferrer">無料相談フォームを開く <b>→</b></a><ul><li>入力は約3分</li><li>相談・概算のご案内は無料</li><li>送信だけで契約にはなりません</li></ul><small>フォームは別タブで開きます。</small></div></div></section>
