...
export default function Home({ initialTarget }: { initialTarget?: string }) {
  ...
  return <main>
    ...
    {!floatDismissed && <aside className={`yFloat ${floatVisible ? "isVisible" : ""} ${floatPulse ? "isPulse" : ""}`} aria-label="無料相談へのご案内"><a className="floatLink floatContact" href="#contact"><small>入力は約3分・相談無料</small><strong>無料で業務と費用感を整理する <b>→</b></strong></a><button className="floatClose" type="button" aria-label="バナーを閉じる" onClick={() => { setFloatVisible(false); setFloatDismissed(true); sessionStorage.setItem("ai-estimate:float-dismissed", "1"); }}>✕</button></aside>}
